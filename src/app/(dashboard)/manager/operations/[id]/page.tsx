...
type Tab = "overview" | "run" | "feed" | "production" | "crew" | "briefings";
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-5">
      ...
      <nav className="scrollbar-elysian flex overflow-x-auto border-b border-charcoal/12" aria-label="Operations workspace sections">
        {([[
          "overview", "Live pulse"], ["run", "Run of show"], ["feed", `Feed · ${pulse.open}`], ["production", "Production dossier"], ["crew", `Crew · ${workspace.shifts.length}`], ["briefings", `Briefings · ${workspace.briefings.length}`]] as [Tab, string][]).map(([value, label]) => <button key={value} type="button" onClick={() => setTab(value)} className={cn("whitespace-nowrap border-b-2 px-5 py-3 font-accent text-[10px] uppercase tracking-[0.18em]", tab === value ? "border-gold-primary text-charcoal" : "border-transparent text-slate hover:text-charcoal")}>{label}</button>)}
      </nav>
