-- Planner deselection must mirror the booking API's financial-history guard.
-- Even an INQUIRY row is retained once a ledger or invoice references it.
  ...
  loop
    if not exists (
      ...
      if exists (
        ...
        where invoice.booking_id = v_booking.id
      ) then
        raise exception
          'A vendor selection with financial history must be changed from bookings'
          using errcode = '55000';
      end if;
